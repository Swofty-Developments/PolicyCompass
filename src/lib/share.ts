...
/** Decode a #r=… share hash back into a standing (or null). Pre-abstain links carry a 2-tally. */
export function decodeShare(hash: string): SharedResult | null {
  try {
    const m = hash.match(/[#&]r=([^&]+)/)
    if (!m) return null
    const obj = JSON.parse(dec(m[1])) as { v: number[]; s?: number[]; t?: number[] }
    ...
    return { vector, stds, tally: [obj.t?.[0] ?? 0, obj.t?.[1] ?? 0, obj.t?.[2] ?? 0] }
  } catch {
    return null
  }
}
