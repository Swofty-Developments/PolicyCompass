...
/** Displayed fervour level: floor of the fractional figure, banded 0..3. */
export const fervourLevel = (fervour: number): number => Math.min(3, Math.max(0, Math.floor(fervour)))

/** Relations banding — one set of thresholds for the correspondence cards
 *  and the house rail's tone words. */
export function relationsBand(r: number): { word: string; line: (short: string) => string } {
  if (r >= 60) return { word: 'devoted', line: (s) => `${s} regards you as one of their own; their lobby is open to you.` }
  if (r >= 30) return { word: 'warm', line: (s) => `Warm letters. ${s} expects to find you in their lobby more often than not.` }
  if (r >= 10) return { word: 'cordial', line: () => 'Cordial terms — favours are possible, at a price.' }
  if (r > -10) return { word: 'formal', line: () => 'Strictly business. They watch how you vote and write accordingly.' }
  if (r > -40) return { word: 'cooling', line: (s) => `The letters have grown short. ${s} counts your votes against them.` }
  if (r > -60) return { word: 'hostile', line: () => 'The letterhead has chilled; what arrives comes unsigned.' }
  return { word: 'severed', line: () => 'Nothing arrives. What passes between you now goes through the newspapers.' }
}
  ...
  return (
    <article className={'t-corr' + chill}>
      ...
      <p className="t-corr-tone">
        {relationsBand(bs.relations).line(bloc.short)} {fervourPhrase(level)}
      </p>
